...
§15 before-after (n=4, recipe A, tierRules): side-by-side BEFORE | AFTER split, SAME person both halves. ⛔ 4 MANDATORY differences per image: (1) PRODUCT — only in AFTER half; BEFORE hands empty, no product anywhere. (2) OUTFIT — different color AND style each half. (3) BACKGROUND — different room/location (not just lighting tweak). (4) EXPRESSION — BEFORE problem-state (tired/self-conscious), AFTER transformed (confident/bright smile). conceptScene MUST follow pattern: "BEFORE (left): [no product, outfit X, setting A, problem-state]. AFTER (right): [holding product, outfit Y, setting B, transformed]." textOverlayBlocks = "Sebelum"/"Selepas" labels + person name+age ("Kak Aishah, 38 tahun") + duration ("Selepas 21 hari guna"). Mix subjectLockKey across 4 images (primary hijab woman + secondary Malay male). Transformation MUST come from identity.transformationByTier — tier1_primary or tier2_axis ONLY. NEVER tier4_offniche (auto off-category for THIS product — examples vary per product: for gut/dental → never body-slimming; for weight-loss product → never hair regrowth; for skincare → never weight-loss; etc).
