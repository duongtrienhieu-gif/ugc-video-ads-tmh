...
- ⚠️ ALL textOverlayBlocks values MUST be in ${langName}. Medical / scientific / technical terms MUST be translated to ${langName} — do NOT default to English. Exceptions ONLY: brand names, product names, prices, certification codes (HALAL/KKM/FDA), social media handles.
...
§12 expert-kol (n=2, recipe H): Image 1 recipeVariant="expert", subjectLockKey="primary" — top portrait of Malaysian specialist MATCHING identity.productCategory (NOT default to dentist — dental specialist ONLY if product is dental). See recipe H mapping for: dental→Pergigian, hair→Trikologi, probiotic→Pemakanan, nasal→ENT, skincare→Dermatologi, joint→Ortopedik, weight→Pemakanan, beauty→Dermatologi, default→Kesihatan. Fill textOverlayBlocks with: (a) DR name "Dr. [UNIQUE Malaysian name]" — vary per pack, mix gender + ethnicity (Malay/Chinese/Indian Malaysian). ⛔ AVOID overused defaults: "Siti Aisyah" / "Aminah" / "Nor Aini" / "Fatimah Zahra". Sample broader pool: Dr. Nurul Hidayah, Dr. Aiman Razak, Dr. Zarina Ibrahim, Dr. Hafiz Ramli, Dr. Marina Tan, Dr. Iskandar Mohd, Dr. Priya Naidu, Dr. Wei Ming Lim. (b) Specialty + experience "[Derived specialty] · [N]+ tahun pengalaman" — N varies in {8, 12, 18, 22, 25, 28} per pack. ⛔ AVOID default 15. (c) 2-4 sentence ${langName} quote endorsing this product. Image 2 recipeVariant="kol", subjectLockKey="secondary" — DIFFERENT person from expert (force secondary identity to avoid same-face bug). Top lifestyle photo Malaysian KOL. Fill textOverlayBlocks with: (a) handle "@[UNIQUE handle]" — vary per pack, mix gender + style. ⛔ AVOID overused: "@adamharith" / "@fatimahzahra_official". Sample broader pool: @nadiaazlan, @putri.lifestyle, @zackeryhakim, @aimi.budak, @imanrazak, @lina.mok, @rashidi.malik. (b) Followers "[N]M Followers / Pengikut" or "[N]K Pengikut" — N varies in {280K, 540K, 780K, 1.8M, 2.4M, 3.2M}. ⛔ AVOID default 1.2M. (c) Casual ${langName} quote with emojis. ⛔ BOTH quotes MUST mention identity.productNameExact VERBATIM at least once. NEVER invent other brand names (no "GlutaGlow", no skincare brand drift, no made-up products). NO product packaging visible in either image. reviews[] = 2 entries matching the 2 images.
...
§13 social-proof (n=6): MIXED recipe + aspect per image —
  ...
  social_group.jpg    — recipeId="B" (clean UGC photo), aspectRatio="4:5" — ONE candid group photo of 4-5 Malaysian family/friends together (mix gender + ages + hijab + uncles + younger). ⛔ EVERY PERSON visibly holds product at chest-level (~10-15% frame per hand, readable, NOT background-tiny) — ALL products are IDENTICAL clones (same shape/label/color/proportions, NEVER variations between hands). Outdoor or casual indoor scene. NO platform chrome, NO caption banner, NO text overlay.
  social_collage.jpg  — recipeId="B", aspectRatio="4:5" — 2x2 grid of 4 separate Malaysian people (different person + different setting per frame). EACH holds product at chest-level (~15-20% per frame, readable) — ALL 4 products are IDENTICAL clones (same shape/label/color, NEVER variations between frames). NO platform chrome, NO caption banner.
...
§15 before-after (n=4, recipe A, tierRules): side-by-side BEFORE | AFTER split, SAME person both halves. ⛔ 4 MANDATORY differences per image: (1) PRODUCT — only in AFTER half; BEFORE hands empty. (2) OUTFIT — different color AND style each half. (3) BACKGROUND — different room/location. (4) PROBLEM REVEAL — BEFORE half visually EXPOSES the problem area per identity.productCategory (dental=mouth OPEN showing damaged/yellow teeth; skin=face close-up showing blemishes; hair=scalp/hair visible showing thinning; gut=hand on stomach in discomfort; joint=hand on painful joint; nasal=red nose/tissue; weight=full-body showing target area). NEVER conceal the problem (closed-mouth dental = pack fail). AFTER: clean result + confident bright smile. conceptScene pattern: "BEFORE (left): [no product, outfit X, setting A, problem REVEALED via Z]. AFTER (right): [holding product, outfit Y, setting B, clean result]." textOverlayBlocks = "Sebelum"/"Selepas" labels + person name+age ("Kak Aishah, 38 tahun") + duration ("Selepas 21 hari guna"). Mix subjectLockKey across 4 images (primary hijab woman + secondary Malay male). Transformation MUST come from identity.transformationByTier — tier1_primary or tier2_axis ONLY. NEVER tier4_offniche (auto off-category for THIS product — examples vary per product: for gut/dental → never body-slimming; for weight-loss product → never hair regrowth; for skincare → never weight-loss; etc).
