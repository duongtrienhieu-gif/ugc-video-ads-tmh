...
/** Which Phase-4 blocks consume payoff sectionFlavor. */
type PayoffBlockId = Extract<BlockId, 'micro-transformation' | 'emotional-wins' | 'future-self-cta'>
...
export interface PayoffArchetype {
  ...
  destination: string
  /** Per-block flavor briefs. Drive Phase-4 block directives.
   *  Each is a concrete WHAT-not-HOW directive — Gemini decides phrasing
   *  via narrator voice. Keys are BlockIds (post-v5.8 Reader-Immersion). */
  sectionFlavor: Record<PayoffBlockId, string>
...
/** Compose payoff brief for per-block prompt injection.
 *  Used in buildPackGenPrompt for Phase-4 blocks that consume payoff flavor. */
export function payoffSectionFlavor(
  payoff: PayoffArchetype,
  blockId: PayoffBlockId,
): string {
  return `PAYOFF ARCHETYPE: ${payoff.id} — destination=${payoff.destination}
  Block-specific flavor: ${payoff.sectionFlavor[blockId]}`
}
...
This drives the FELT TONE of Phase-4 blocks (micro-transformation /
emotional-wins / future-self-cta). KHÔNG default "peaceful reflection" —
every pack ends in a different emotional place. Narrator voice still
drives phrasing.`
}
