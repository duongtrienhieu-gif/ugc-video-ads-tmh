...
export interface StorytellingMeta {
  ...
  // ── PI image plans (2026-05-30) ───────────────────────────────────
  // Parallel store for PI-block images so we don't pollute the
  // composer-driven `exportablePage.sections` chain with PI types.
  // Keyed by full PI block id (`pi-<type>`, see piBlockIdForType).
  //
  // Architecture rationale:
  //   - Storytelling composed sections live in exportablePage.sections
  //     because they participate in the composer/renderContract/
  //     orchestration subtype chain (consumed by serializers,
  //     ladipage adapter, etc.).
  //   - PI blocks are PARALLEL — generated by a separate Gemini call
  //     and interleaved into the pack at anchor positions. They do NOT
  //     have ComposedSection / RendererAdaptedSection metadata.
  //   - Putting them in exportablePage would force downstream consumers
  //     to special-case PI types or invent fake values for ~40 fields.
  //   - This parallel store keeps the type chain clean while still
  //     letting the image gen pipeline plan + execute + render PI images.
  //
  // Currently only `pi-mechanism-personal` populates these maps (it's
  // the only PI type with PI_IMAGE_ROLE !== null). The map shape allows
  // future PI types to opt in without further schema changes.
  /** Pre-computed scene prompt per PI block id. */
  piImageScenes?: Record<string, import('../imageSceneSynthesis').SceneDescription>
  /** Generation plan + executed asset per PI block id. */
  piImageAssets?: Record<string, import('../generationOrchestration').GeneratedAsset>
}
