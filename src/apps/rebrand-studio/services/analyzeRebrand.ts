...
export async function analyzeRebrand(params: AnalyzeRebrandParams): Promise<RebrandIdentity> {
  ...
  const systemInstruction =
    ...
    `   • If this is a DIETARY SUPPLEMENT / vitamin / herbal remedy in TABLET / CAPSULE / SOFTGEL / PILL / powder-sachet form → nutritionTitle = "SUPPLEMENT FACTS"; nutrition = a Supplement Facts panel that STARTS with "Serving size: <the real dose, e.g. 1 tablet (290mg)>", then lists EACH active ingredient with its amount PER SERVING. For the %NRV/RDI column, follow these rules EXACTLY:\n` +
    `       - VITAMINS & MINERALS have an official Nutrient Reference Value → give a realistic estimated number, e.g. "Vitamin B1 1.2mg (109% NRV)", "Folic Acid 300mcg (150% NRV)".\n` +
    `       - BOTANICAL / HERBAL extracts have NO established NRV (e.g. Black Sesame, Ganoderma, Angelica, Mulberry, Ginseng) → write the amount then "(†)" — NEVER invent a % for these, e.g. "Black Sesame 200mg (†)".\n` +
    `       - If AT LEAST ONE ingredient used "(†)", append a final line exactly: "† Daily Value not established."\n` +
    `       - If EVERY ingredient is a botanical with no NRV, DROP the %NRV column and the † footnote entirely — just list "Ingredient  Amount" (two columns), with NO (†) marks at all.\n` +
    `     NEVER output placeholder asterisks like "**% NRV". Do NOT output Energy/Protein/Fat/Carbohydrate for supplements — they are meaningless for a tablet.\n` +
