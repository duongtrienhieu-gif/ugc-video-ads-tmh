  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <button
        ...
        className="group flex h-28 w-28 items-center justify-center rounded-xl border border-dashed border-black/10 bg-black/[0.02] transition-colors hover:border-black/15 overflow-hidden"
      >
        {displayImage ? (
      ...
      <label className="flex flex-col gap-1">
        <span className="text-[11px] font-medium uppercase tracking-widest text-gray-500">Tên *</span>
        <input
          value={name}
      ...
      <label className="flex flex-col gap-1">
        <span className="text-[11px] font-medium uppercase tracking-widest text-gray-500">Ghi chú</span>
        <textarea
          value={notes}
      ...
      <div className="rounded-xl border border-black/8 bg-black/[0.02]">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          >
            ...
            <span className="text-xs font-medium text-gray-700">Character DNA (Dán JSON)</span>
            {jsonExpanded ? (
          ...
          {jsonInput.trim() && (
            <button
              ...
              className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-medium text-gray-500 transition-colors hover:bg-black/5 hover:text-gray-700"
            >
        ...
        {jsonExpanded && (
          <div className="border-t border-black/8 px-4 py-3">
            <textarea
              ...
              rows={8}
              placeholder={'{\n  "Physical": {\n    "gender": "Female"\n  }\n}'}
              className={`w-full rounded-lg border bg-gray-100 p-3 text-[11px] font-mono leading-relaxed text-gray-600 outline-none transition-colors resize-y ${jsonError ? 'border-red-500/50 focus:border-red-400' : 'border-transparent focus:border-black/15'
                }`}
            />
