      ...
      {GROUP_META.map((group) => {
        ...
        return (
          <section key={group.group} className="flex flex-col gap-2">
            ...
            <p className="text-[11px] text-gray-500">{group.description.vi}</p>
...
function AssetCard({ entry, groupAccentClass, groupCardBgClass, selected, onSelect }: AssetCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`group flex flex-col items-start gap-1.5 rounded-xl border px-3 py-3 text-left transition-all ${
        selected
    ...
    >
      <div className="flex w-full items-center justify-between">
        <Icon className={`h-5 w-5 ${selected ? 'text-violet-600' : 'text-gray-500 group-hover:text-gray-800'}`} strokeWidth={1.6} />
      ...
      <span className={`mt-1 inline-flex rounded px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${
